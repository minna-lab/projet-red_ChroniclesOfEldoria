...
func patternGobelin(gobelin *Monstre, joueur *Personnage, tour int) {
	...
	if tour%3 == 0 {
		...
		fmt.Printf(ROUGE+"💥 %s rugit et prépare une attaque dévastatrice !\n"+RESET, gobelin.Nom)
	}
...
func tourJoueur(joueur *Personnage, gobelin *Monstre) bool {
	...
	for {
		fmt.Println(POURPRE + "\n╔══════════════════════════════════════╗")
		fmt.Println(POURPRE + "║         VOTRE TOUR, BRAVE HÉROS !    ║" + RESET)
		fmt.Println(POURPRE + "╠══════════════════════════════════════╣" + RESET)
		fmt.Println(BLEU + "║  1. 🗡️  Attaque basique (5 dégâts)   ║" + RESET)
		fmt.Println(VERT + "║  2. ✨ Lancer un sort                 ║" + RESET)
		fmt.Println(JAUNE + "║  3. 🎒 Utiliser l'inventaire          ║" + RESET)
		fmt.Println(BLANC + "║  0. 🏃 Fuir (retour au menu)          ║" + RESET)
		fmt.Println(POURPRE + "╚══════════════════════════════════════╝" + RESET)
		...
		case 3:
			...
			fmt.Println(POURPRE + "\n╔══════════════════════════════════════╗")
			fmt.Println(POURPRE + "║            🎒 VOTRE INVENTAIRE       ║" + RESET)
			fmt.Println(POURPRE + "╠══════════════════════════════════════╣" + RESET)
			for i, item := range joueur.Inventaire {
				fmt.Printf(BLEU+"║  %d. %s\n"+RESET, i+1, item)
			}
			fmt.Println(BLANC + "║  0. Annuler                          ║" + RESET)
			fmt.Println(POURPRE + "╚══════════════════════════════════════╝" + RESET)
			var choixItem int
			fmt.Print(VERT + "➤ Votre choix : " + RESET)
			fmt.Scan(&choixItem)
			...
			return false
		case 0:
			fmt.Println(JAUNE + "🏃 Vous battez en retraite... La prudence est mère de sûreté !" + RESET)
			return true
		default:
			fmt.Println(ROUGE + "❌ Choix invalide." + RESET)
		}
...
func utiliserItemCombat(p *Personnage, item string, index int) {
	switch item {
	case "Potion de Vie":
		fmt.Printf(VERT+"🧪 %s boit une Potion de Vie en plein combat !\n"+RESET, p.Nom)
...
func lancerSort(joueur *Personnage, gobelin *Monstre) bool {
	...
	if joueur.ManaActuel < cout {
		fmt.Printf(BLEU+"💧 Mana insuffisant ! Il vous faut %d mana (vous avez %d).\n"+RESET, cout, joueur.ManaActuel)
		return false
	}
	...
	fmt.Printf(VERT+"✨ %s invoque '%s' : %d dégâts magiques à %s !\n"+RESET, joueur.Nom, sort, dgts, gobelin.Nom)
	fmt.Printf(JAUNE+"👹 %s HP : %d/%d\n"+RESET, gobelin.Nom, gobelin.HPActuel, gobelin.HPMax)
	return true
}
...
func combatEntrainement(joueur *Personnage) {
	...
	fmt.Printf(JAUNE+"%s VS %s\n"+RESET, joueur.Nom, gobelin.Nom)

	joueurCommence := joueur.Initiative >= gobelin.Initiative
	if joueurCommence {
		fmt.Printf(VERT+"🎲 %s a plus d'initiative et attaque en premier !\n"+RESET, joueur.Nom)
	} else {
	...
	for {
		fmt.Printf(ROUGE+"\n--- TOUR %d ---\n"+RESET, tour)
		fmt.Printf(VERT+"❤️  %s : %d/%d"+RESET+" | "+ROUGE+"👹 %s : %d/%d\n"+RESET,
...
func victoire(joueur *Personnage, gobelin *Monstre) {
	fmt.Println(JAUNE + "\nVICTOIRE !" + RESET)
	fmt.Printf(JAUNE+"🏆 Vous avez vaincu %s !\n"+RESET, gobelin.Nom)
	gagnerXP(joueur, 30)
}
...
func gagnerXP(p *Personnage, xp int) {
	fmt.Printf(JAUNE+"⭐ Vous gagnez %d points d'expérience !\n"+RESET, xp)
	p.XPActuel += xp
	for p.XPActuel >= p.XPMax {
		...
		fmt.Printf(JAUNE+"🎉 NIVEAU %d ! HP max : %d | Mana max : %d\n"+RESET, p.Niveau, p.HPMax, p.ManaMax)
	}
	fmt.Printf(JAUNE+"📊 XP : %d/%d\n"+RESET, p.XPActuel, p.XPMax)
}
